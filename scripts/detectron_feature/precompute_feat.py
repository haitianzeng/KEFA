...
from detectron import Detector
...
def build_tsv():
    ...
    for scanId, viewpointId in viewpointIds:
        ...
        for ix in range(VIEWPOINT_SIZE):
            ...
            # Transform from rgb to bgr (Detectron takes bgr input)
            img_list.append(cvmat_to_numpy(state.rgb)[:, :, ::-1])

        t_render.toc()
        t_net.tic()

        with open('./data/v1/features/detectron_1/{}_{}.pkl'.format(scanId, viewpointId), 'wb') as pklfile:
