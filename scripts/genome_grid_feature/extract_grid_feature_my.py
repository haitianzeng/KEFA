...
def build_tsv():
    ...
    # initialize detector visual genome
    from extract_grid_feature import setup, extract_grid_feature_argument_parser
    ...
    dir = './data/v1/features/genome/X-101.pth'
    DetectionCheckpointer(model, save_dir='./output').resume_or_load(
        './data/v1/features/genome/X-101.pth', resume=True
    )
    ...
    for scanId, viewpointId in viewpointIds:
        ...
        # with open('./data/v1/features/genome_grid_feat/{}_{}.pth'.format(scanId, viewpointId), 'wb') as pklfile:
        #     pickle.dump(detector.get_roi_feature(img_list), pklfile)
        do_single_extract(model, img_list, './data/v1/features/genome_grid_feat/{}_{}.pth'.format(scanId, viewpointId))
...
def do_single_extract(model, image_list, save_path):
    with torch.no_grad():
        # build inputs
        inputs = []
