...
from utils import load_nav_graphs, Tokenizer
        ...
        for split in splits:
            for item in load_datasets_FGR2R([split]):
                ...
                for j, instr in enumerate(item['instructions']):
                    ...
                    # Sub-instruction annotation

                    # debug: some path have 4 or more instructions. Keep the first 3 instructions.
                    # print(len(item['chunk_view']), len(item['instructions']))
                    # if (len(item['chunk_view']) != len(item['instructions'])):
                    #     print(item['chunk_view'])
                    #     print(item['instructions'])
                    #     print(item['path_id'])

                    # only first three instructions are annotated, if there are more than three instructions
                    # if len(item['instructions']) > 3 and j == 3:
                    #     break
                    if split == 'train' and j >= 3:
                        continue

                    if split == 'train':
                        # new_item['new_instructions'] = yaml.safe_load(item['new_instructions'])[j]
                        # print(new_item['new_instructions'])
                        # exit(0)
                        # new_item['chunk_view'] = item['chunk_view'][j]
                        new_item['obj_vg_class_list'] = eval(item['obj_vg_class_list'])[j]
                        new_item['obj_word_vp_list'] = eval(item['obj_word_vp_list'])[j]
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            obs.append({
                ...
                'obj_vg_class_list': item['obj_vg_class_list'] if 'obj_vg_class_list' in item else None,
                'obj_word_vp_list': item['obj_word_vp_list'] if 'obj_word_vp_list' in item else None,
                'subins_pos_list': item['subins_pos_list'] if 'subins_pos_list' in item else None
            })
            ...
            if obs[-1]['teacher'] not in [can['viewpointId'] for can in obs[-1]['candidate']] and obs[-1]['teacher'] != obs[-1]['viewpoint']:
...
def load_datasets_FGR2R(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            if split == 'train':
                with open('tasks/OBJ_FGR2R/data/OBJVGv3_FGR2R_%s.json' % split) as f:
                    new_data = json.load(f)
            else:
                with open('tasks/OBJ_FGR2R/data/OBJVG_FGR2R_%s.json' % split) as f:
                    new_data = json.load(f)
            #with open('tasks/FGR2R/data/FGR2R_%s.json' % split) as f:
            #with open('tasks/OBJ_FGR2R/data/OBJVGv3_FGR2R_%s.json' % split) as f:
            #    new_data = json.load(f)
        else:
