...
def load_nav_graphs(scans):
    ...
    graphs = {}
    for scan in scans:
        with open('D:/MyFiles/Projects/EnvDrop_my/connectivity/%s_connectivity.json' % scan) as f:
            G = nx.Graph()
            positions = {}
            data = json.load(f)
...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            with open('D:/MyFiles/Projects/EnvDrop_my/tasks/R2R/data/R2R_%s.json' % split) as f:
...
def add_exploration(paths):
    explore = json.load(open("D:/MyFiles/Projects/EnvDrop_my/tasks/R2R/data/exploration.json", 'r'))
