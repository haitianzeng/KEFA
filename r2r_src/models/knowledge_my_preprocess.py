...
class FactEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, max_len, embedding_size, hidden_size, input_dropout_p, dropout_p, n_layers, rnn_cell='lstm',
...
class Knowledge_attention(nn.Module):
    def __init__(self, d_model=256, vis_dim=2048, fact_dim=256):
        super(Knowledge_attention, self).__init__()
...
def request_entity_obj():
    ...
    entity_path = '../KB/data/entities.txt'
    ...
    with open('obj_cache.pkl', 'wb') as f:
        pickle.dump(obj_cache, f)


def process_entity_knowledge():
    with open('obj_cache.pkl', 'rb') as f:
    ...
    glove_embeds = {}
    glove_embeds['<unk>'] = 'na'  # saved for embedding unknown words
    ...
    for i, entity in enumerate(core_entities):
        ...
        for edge in edges:
            ...
            tmp = []
            tmp.extend(edge['start']['label'].replace('_', ' ').split(' '))
            tmp.extend(re.findall('([A-Z][a-z]*)', edge['rel']['label']))
            tmp.extend(edge['end']['label'].replace('_', ' ').split(' '))
            tmp = [a.lower() for a in tmp]
    ...
    # embedding_path = '../KB/data/glove.840B.300d.txt'
    ...
    glove_embeds['<PAD>'] = np.zeros(300)
    glove_embeds['<BOS>'] = np.zeros(300)
    glove_embeds['<EOS>'] = np.zeros(300)
    glove_embeds['<BOS>'][:150] = 1
    glove_embeds['<BOS>'][150:] = 1

    with open('knowledge_plus_embed.pkl', 'wb') as f:
...
def main():
    exit(0)
    cache_path = 'D:\\MyFiles\\Projects\\CKR-main\\KB\\cache\\conceptnet.cache'
    ...
    embedding_path = 'D:\\MyFiles\\Projects\\CKR-main\\KB\\cache\\embedding.cache'
...
def build_fact_vocab():
    ...
    with open('knowledge_rel_embed.pkl', 'wb') as f:
...
def process_entity_knowledge_v2():
    with open('obj_cache.pkl', 'rb') as f:
        obj_cache = pickle.load(f)
    ...
    for i, entity in enumerate(core_entities):
        ...
        for edge in edges:
            ...
            tmp = []
            st = edge['start']['label'].replace('_', ' ').split(' ')
            st = [word.lower() for word in st]
            tmp.append(st)
            rel = re.findall('([A-Z][a-z]*)', edge['rel']['label'])
            rel = [word.lower() for word in rel]
            tmp.append(rel)
            ed = edge['end']['label'].replace('_', ' ').split(' ')
            ed = [word.lower() for word in ed]
            tmp.append(ed)
    ...
    with open('knowledge_plus_embed_v2.pkl', 'wb') as f:
        pickle.dump([knowledge_dict, glove_embeds], f)


def build_fact_vocab_v2():
    with open('knowledge_plus_embed_v2.pkl', 'rb') as f:
        knowledge_dict, glove_embeds = pickle.load(f)
    ...
    for k, v in knowledge_dict.items():
        relation_embeds = []
        for relation in v:
            st = glove_embeds[relation[0][-1]]  # last word of the start entity

            # average word embedding of relation words
            rel_ = np.zeros((300))
            for rel_word in relation[1]:
                rel_ += glove_embeds[rel_word]
            if len(relation[1]) > 0:
                rel = rel_ / len(relation[1])

            ed = glove_embeds[relation[2][-1]]  # last word of the end entity

            relation_embeds.append(torch.from_numpy(np.concatenate((st, rel, ed), axis=0)).float())

        if len(relation_embeds) == 0:
            relation_embeds.append(torch.zeros((900)))

        relation_embeds = torch.stack(relation_embeds, dim=0)
        if len(relation_embeds) > num_fact:
            relation_embeds = relation_embeds[:num_fact, :]
        elif len(relation_embeds) < num_fact:
            relation_embeds = torch.cat((relation_embeds, torch.zeros((num_fact - len(relation_embeds), 900))), dim=0)

        assert relation_embeds.shape[0] == num_fact
        assert relation_embeds.shape[1] == 900

        knowledge_dict_new[k] = relation_embeds

    with open('knowledge_rel_embed_v2.pkl', 'wb') as f:
        pickle.dump(knowledge_dict_new, f)



def process_entity_knowledge_v3():
    with open('obj_cache.pkl', 'rb') as f:
        obj_cache = pickle.load(f)
    ...
    with open('knowledge_plus_embed_v3.pkl', 'wb') as f:
        pickle.dump([knowledge_dict, glove_embeds], f)


def build_fact_vocab_v3():
    with open('knowledge_plus_embed_v3.pkl', 'rb') as f:
        knowledge_dict, glove_embeds = pickle.load(f)
    ...
    with open('knowledge_rel_embed_v3.pkl', 'wb') as f:
        pickle.dump(knowledge_dict_new, f)
...
if __name__ == '__main__':
    # main()
    # request_entity_obj()
    # process_entity_knowledge()
    # build_fact_vocab()
    # test_fact_encoder()
    # process_entity_knowledge_v2()
    # build_fact_vocab_v2()
    # process_entity_knowledge_v3()
    build_fact_vocab_v3()
